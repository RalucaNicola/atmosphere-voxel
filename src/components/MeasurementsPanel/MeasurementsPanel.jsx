...
const MeasurementsPanel = ({
  ...
  return (
    <div>
      <Background>
        <Title text='Measurements' size='small'></Title>
          ...
          {variables.map((variable, index) => {
            ...
            return (
              <CalciteLabel key={index} layout='inline' className={styles.label}>
                <CalciteRadioButton value={variable.name} {...checked}></CalciteRadioButton>
                {variable.name}
              </CalciteLabel>
            );
          })}
          ...
          {selectedVariable.visualization.length > 1 ? (
            ...
            >
              {selectedVariable.visualization.map((visualization, index) => {
                ...
                return (
                  <CalciteLabel key={index} layout='inline' className={styles.label}>
                    <CalciteRadioButton value={visualization.name} {...checked}></CalciteRadioButton>
                    {visualization.name}
                  </CalciteLabel>
                );
              })}
